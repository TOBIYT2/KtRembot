...
let handler = async (m, { conn: _conn, args, usedPrefix, command, isOwner }) => {
  ...
  async function serbot() {
    ...
    // 🛠️ Corrección aquí: solo decodifica si NO es número con +
    if (args[0] && !args[0].startsWith('+')) {
      try {
        let data = JSON.parse(Buffer.from(args[0], "base64").toString("utf-8"));
        fs.writeFileSync(`${userFolderPath}/creds.json`, JSON.stringify(data, null, '\t'));
      } catch (e) {
        console.error('❌ Error al intentar decodificar el base64:', e);
      }
    }
    ...
    if (methodCode && !conn.authState.creds.registered) {
      ...
      setTimeout(async () => {
        ...
        codeBot = codeBot?.match(/.{1,4}/g)?.join("-") || codeBot;
            ...
            txt += `│  🏎️  Pasos\n`
            txt += `│  🏎️  *1* : Haga click en los 3 puntos\n`
            txt += `│  🏎️  *2* : Toque dispositivos vinculados\n`
            txt += `│  🏎️  *3* : Selecciona *Vincular con el número de teléfono*\n` 
            txt += `└  🏎️  *4* : Escriba el Codigo\n\n`
